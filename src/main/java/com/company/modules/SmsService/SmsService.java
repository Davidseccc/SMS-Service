package com.company.modules.SmsService;

import com.company.ComPortSendReceive;
import com.company.JSerialComm;
import com.company.modules.SmsService.iface.SmsEventListener;
import com.company.modules.SmsService.iface.SmsServiceInterface;
import com.company.modules.SmsService.model.PDU;
import com.company.modules.SmsService.model.SMS;
import com.company.modules.SmsService.utils.HTTPReader;
import com.company.modules.SmsService.utils.PhoneNumberValidator;
...
public class SmsService implements SmsServiceInterface {
    ...
    private List<SmsEventListener> listeners = new ArrayList<>();
    HTTPReader factory;
    ...
    public void propagateEvent(SmsEvent e) {    //module neco delej, tj CORE->MODULE
        ...
        if (e.getEventType() == SmsEvent.SmsEventType.MESSAGE_SEND) {
            List<SMS> smsList = (List<SMS>) e.getEventData();
            for (SMS sms :smsList) {
                factory.sendSMS(sms.getSender(),sms.getMessage());
            }
        }
        if (e.getEventType() == SmsEvent.SmsEventType.LIST_SMS) {
            int SMS_TYPE = ((int) e.getEventData());
            List<PDU> list = factory.getAllSMS();
            for (PDU p:list) {
